...
static CGFloat const kBButtonCornerRadiusV3 = 1.0f;
...
- (void)drawBButtonStyleV3InRect:(CGRect)rect withContext:(CGContextRef *)context
{
    ...
    /*
    UIColor *border = (!self.highlighted) ? [self.color bb_darkenColorWithValue:0.06f] : [self.color bb_darkenColorWithValue:2.0f];
    if (!self.enabled) {
        [border bb_desaturatedColorToPercentSaturation:0.60f];
    }
